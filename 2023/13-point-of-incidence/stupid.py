...
def elementwise_eq(row_a, row_b, tolerance=0):
    ...
    return (True, tolerance-smudge_count)
...
def horizontal_pattern_score(pattern, magic, tolerance):
    ...
    for i,row in enumerate(pattern):
        if prev_row != None:
            result, new_tolerance = elementwise_eq(prev_row, row, tolerance)
            if result:
                j,k = i-1,i
                found_nonmatching = False
                while j >= 0 and k < len(pattern):
                    result, new_tolerance = \
                        elementwise_eq(prev_row, row, new_tolerance)
                    if result:
                        j -= 1
                        k += 1
                    else:
